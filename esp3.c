...
ESP3_RETURN_VALUES_t esp3_ReceivePacket(ESP3_TELEGRAM_t *telegram)
{
	...
	uint8_t *u8Raw = (uint8_t*)telegram;
	// Temporal variable
	uint8_t i;
	uint16_t check = 1;
	uint8_t *little_endian = (uint8_t*)&check;

	// State machine goes on when a new byte is received
	while (hal_UartGetByte(&u8RxByte) == OK)
	{	
		...
		{	
			...
			case GET_SYNC_STATE:
				if (u8RxByte == SER_SYNCH_CODE) 	//u8RxByte == 0x55
				{
			...
			case CHECK_CRC8H_STATE:
				...
				// Check endian and change the code
   				if (*little_endian)	telegram->u16DataLength = (((u8Raw[0] & 0xff) << 8) | (u8Raw[1] & 0xff));

				// CRC8H correct. Length fields values valid?
				
				if((telegram->u16DataLength + telegram->u8OptionLength) == 0)
				{
			...
			case GET_DATA_STATE:
				...
				if(u16Count < telegram->u16BufferLength)
				{
					telegram->u8DataBuffer[u16Count] = u8RxByte;
					u8CRC = proc_crc8(u8CRC, u8RxByte);
				}
				// When all expected bytes received, go to calculate data checksum
				if( ++u16Count == (telegram->u16DataLength + telegram->u8OptionLength) )
				{
			...
			case CHECK_CRC8D_STATE:
				...
				if (u16Count > telegram->u16BufferLength) return ESP3_BUFFER_TO_SHORT;
				// Enough space to allocate packet. Equals last byte the calculated CRC8?
				if (u8CRC == u8RxByte) return ESP3_OK; // Correct packet received
				// False CRC8.
	...
	return (u8State == GET_SYNC_STATE) ? ESP3_NO_RX_BYTE : ESP3_ONGOING_TELEGRAM;
}
...
{	
	...
	if((telegramToSend->u16DataLength || telegramToSend->u8OptionLength) == 0)
	{
		return ESP3_INVALID_PACKET;
	} // Sync
	while(hal_UartSendByte(SER_SYNCH_CODE) != OK);
	// Header
	if(*little_endian)
	{
		while(hal_UartSendByte((uint8_t)(telegramToSend->u16DataLength >> 8))&&
		  	  hal_UartSendByte((uint8_t)(telegramToSend->u16DataLength & 0xFF))&&
		  	  hal_UartSendByte(telegramToSend->u8OptionLength)&&
		  	  hal_UartSendByte(telegramToSend->type)!= OK);
	}
	else
	{
		while(hal_UartSendByte((uint8_t)(telegramToSend->u16DataLength & 0xFF))&&
		  	  hal_UartSendByte((uint8_t)(telegramToSend->u16DataLength >> 8))&&
		  	  hal_UartSendByte(telegramToSend->u8OptionLength)&&
		  	  hal_UartSendByte(telegramToSend->type)!= OK);
	}
	// Header CRC
	u8CRC = 0;
	u8CRC = proc_crc8(u8CRC, (uint8_t)(telegramToSend->u16DataLength >> 8));
	u8CRC = proc_crc8(u8CRC, (uint8_t)(telegramToSend->u16DataLength & 0xFF));
	u8CRC = proc_crc8(u8CRC, telegramToSend->u8OptionLength);
	u8CRC = proc_crc8(u8CRC, telegramToSend->type);
	while(hal_UartSendByte(u8CRC) != OK);
	// Data
	u8CRC = 0;
	for (i = 0 ; i < (telegramToSend->u16DataLength + telegramToSend->u8OptionLength) ; i++)
	{
		u8CRC = proc_crc8(u8CRC, telegramToSend->u8DataBuffer[i]);
		while(hal_UartSendByte(telegramToSend->u8DataBuffer[i]) != OK);
	}
	// Data CRC
	while(hal_UartSendByte(u8CRC)!=OK);
	return ESP3_OK;
